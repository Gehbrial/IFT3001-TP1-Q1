...
bool test_statistique(unsigned int degre,
  ...
  std::cout << "Execution de " << nb_instances << " instances de division" << std::endl;
  ...
  std::cout << "+" << score * 5.0 / nb_instances << " points (" << score << " instances sur " << nb_instances << ")" << std::endl;
  return true;
}
...
int main(void) {
  ...
  test_statistique(7, 1000, 42, test_generique_division); 
  // Un autre germe sera utilise lors de la correction
  // Aucun polynome de degre superieur a 4 ne sera teste, car il y a un risque de depassement d'entier.
  test_statistique(4, 1000, 666, test_generique_plus_grand_commun_diviseur);
