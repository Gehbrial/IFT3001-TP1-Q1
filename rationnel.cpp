...
// https://fr.wikipedia.org/wiki/Dépassement_d%27entier
...
int Rationnel::plus_grand_commun_diviseur(int a, int b) {
  assert(a >= 0 && b >= 0 && (a > 0 || b > 0)); // Voir note la note sur les depassements d'entier en haut du fichier
  while (b > 0) {
    int reste = a % b;
    a = b;
    b = reste;
...
void Rationnel::simplifier() {
  ...
  m_numerateur /= pgcd;
  m_denominateur /= pgcd;
  assert(m_denominateur > 0); // Voir note la note sur les depassements d'entier en haut du fichier
